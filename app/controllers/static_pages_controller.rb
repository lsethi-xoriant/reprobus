class StaticPagesController < ApplicationController
  ...
  def successful_pin_payment
    @setting = Setting.global_settings 
    @itinerary_price = ItineraryPrice.find(params[:id])
    @itinerary = @itinerary_price.itinerary
    @chargetoken = params[:charge_token]
    
    @charge = PinPayment::Charge.find(params[:charge_token])
    
    @itinerary_price.booking_confirmed = true
...
#    end
  ...
  def dashboard_list
    @user = current_user
    @assigned_enquiries = @user.assigned_enquiries.active.page(params[:page]).per(20)
    render :layout => "application"
  end
